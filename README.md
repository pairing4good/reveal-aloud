...
| --- | --- | --- |
| `engine` | `'webspeech'` | `'webspeech'` (the OS's voices) or `'kokoro'` (see below). |
...
## 6. Better system voices (Mac, free, worth it)
...
load and audition Kokoro (see section 5) without editing a config file first.
...
naming a Siri voice: Apple reserves those and never exposes them to browsers. See step 6.

**It reads a bit robotically.** Try a Premium voice (step 6). If that is still not good enough — some machines just don't have a good one — Kokoro (step 5) is the real fix.
